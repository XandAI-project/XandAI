...
describe('ChatController', () => {
    ...
    beforeEach(async () => {
        ...
        const module = await testing_1.Test.createTestingModule({
            ...
            providers: [{ provide: chat_use_case_1.ChatUseCase, useValue: mockChatUseCase }],
        }).compile();
    ...
    describe('sendMessageStream', () => {
        it('should stream response via SSE', async () => {
            ...
            await controller.sendMessageStream(mockRequest, { content: 'Hello' }, mockResponse);
    ...
    describe('sendMessageToSession', () => {
        it('should send message to specific session', async () => {
            const result = await controller.sendMessageToSession(mockRequest, 'session-123', { content: 'Hello', model: 'llama3.2' });
    ...
    describe('addMessageToSession', () => {
        it('should add message to session', async () => {
            const result = await controller.addMessageToSession(mockRequest, 'session-123', { content: 'Test', role: 'user' });
            expect(result.id).toBe('message-123');
            expect(mockChatUseCase.addMessageToSession).toHaveBeenCalledWith('user-123', 'session-123', 'Test', 'user');
        });
    });
    describe('getSessionMessages', () => {
        it('should return session messages', async () => {
            const result = await controller.getSessionMessages(mockRequest, 'session-123', 1, 50);
    ...
    describe('createOrUpdateMessage', () => {
        it('should create or update message', async () => {
            const result = await controller.createOrUpdateMessage(mockRequest, 'message-123', { id: 'message-123', content: 'Updated', role: 'user' });
            expect(result.id).toBe('message-123');
        });
    });
    describe('attachImageToMessage', () => {
        it('should attach image to message', async () => {
            const result = await controller.attachImageToMessage(mockRequest, 'message-123', { imageUrl: 'http://example.com/image.png', filename: 'image.png' });
            expect(result.id).toBe('message-123');
            expect(mockChatUseCase.attachImageToMessage).toHaveBeenCalledWith('user-123', 'message-123', 'http://example.com/image.png', 'image.png', undefined, undefined);
        });
    });
});
